...
import {
  ...
  ExternalLink, Download, FileCode2, Layers, Code2, Eye,
} from "lucide-react";
...
export function BrowserPreview({
  ...
  // When generating a new project, reset view to preview
  React.useEffect(() => {
    if (isLoading) {
      setViewMode("preview");
      setLiveHtml(null);
          ...
          >
            {currentFiles.map((f) => (
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-secondary/70 hover:bg-secondary border border-border/50 text-xs text-muted-foreground hover:text-foreground whitespace-nowrap shrink-0 font-mono transition-colors"
              >
